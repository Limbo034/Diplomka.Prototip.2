    ...
    private void AutoLogin()
    {
        if (user != null)
        {
            if (user.IsEmailVerified)
            {
                ...
                ManagerMenu.Instance.OpenProfilePanel(); // Открываем ProfilePanel при успешном автологине и верифицированном email

                if (!string.IsNullOrEmpty(user.PhotoUrl?.ToString())) // Проверяем на null перед обращением
                {
        ...
        {
            ManagerMenu.Instance.OpenLoginPanel(); // Открываем LoginPanel, если нет залогиненного пользователя
        }
    }
    #endregion
